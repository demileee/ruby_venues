# find the possible venues that are wheelchair accessible, in Toronto, and can
# fit at least 150 people. These results should be stored in an array.
...
wheelchair = venues.select { |venue| venue[:wheelchair_accessible] == true }
toronto = wheelchair.select { |venue| venue[:city] == "Toronto" }
filtered = toronto.select { |venue| venue[:capacity] >= 150 }
puts filtered
